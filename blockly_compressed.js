...
(function(a) {
	...
	Blockly.utils.userAgent.MOBILE = !Blockly.utils.userAgent.TABLET && (Blockly.utils.userAgent.IPOD || Blockly.utils.userAgent.IPHONE || Blockly.utils.userAgent.ANDROID ||
	...
	switch (a.deltaMode) {
		...
		case 1:
			...
			return {
				...
				x: a.deltaX * Blockly.LINE_MODE_MULTIPLIER, y: a.deltaY * Blockly.LINE_MODE_MULTIPLIER
		...
		case 2:
			...
			return {
				...
				x: a.deltaX * Blockly.PAGE_MODE_MULTIPLIER, y: a.deltaY * Blockly.PAGE_MODE_MULTIPLIER
...
Blockly.utils.screenToWsCoordinates = function(a, b) {
	...
	c = new Blockly.utils.Coordinate(c - e.left, d - e.top);

	d = a.getOriginOffsetInPixels();

	return Blockly.utils.Coordinate.difference(c, d).scale(1 / a.scale)
...
Blockly.Events.filter = function(a, b) {
	...
	for (var d = [], e = Object.create(null), f = 0, g; g = c[f]; f++)
		...
		if (!g.isNull()) {
			...
			else if (g.type == Blockly.Events.MOVE && k.index == f - 1) l.newParentId = g.newParentId, l.newInputName = g.newInputName, l.newCoordinate = g.newCoordinate, k.index = f;

			else if (g.type == Blockly.Events.CHANGE && g.element == l.element && g.name == l.name) l.newValue = g.newValue;
...
Blockly.Events.Create = function(a) {
	...
	a && (Blockly.Events.Create.superClass_.constructor.call(this, a), this.xml = a.workspace.rendered ? Blockly.Xml.blockToDomWithXY(a) : Blockly.Xml.blockToDom(a), this.ids = Blockly.Events.getDescendantIds(a))
...
Blockly.utils.dom.containsNode = function(a, b) {
	...
	return !!(a.compareDocumentPosition(b) & Blockly.utils.dom.Node.DOCUMENT_POSITION_CONTAINED_BY)
	...
	try {
		...
		for (var h = 0; h < e; h++) {
			...
			if ("block" == l || "shadow" == l && !Blockly.Events.recordUndo) {
				...
				if ("comment" == l) b.rendered ? Blockly.WorkspaceCommentSvg ? Blockly.WorkspaceCommentSvg.fromXml(k, b, d) : console.warn("Missing require for Blockly.WorkspaceCommentSvg, ignoring workspace comment.") : Blockly.WorkspaceComment ?

					Blockly.WorkspaceComment.fromXml(k, b) : console.warn("Missing require for Blockly.WorkspaceComment, ignoring workspace comment.");
		...
		f || Blockly.Events.setGroup(!1), Blockly.utils.dom.stopTextWidthCache()
...
Blockly.Xml.domToBlockHeadless_ = function(a, b) {
	...
	for (var g; g = a.childNodes[e]; e++)
		...
		if (g.nodeType != Blockly.utils.dom.Node.TEXT_NODE) {
			...
			for (var h = f = null, k = 0, l; l = g.childNodes[k]; k++) l.nodeType == Blockly.utils.dom.Node.ELEMENT_NODE && ("block" == l.nodeName.toLowerCase() ? f = l : "shadow" == l.nodeName.toLowerCase() && (h = l));
			...
			switch (g.nodeName.toLowerCase()) {
				...
				case "comment":
					...
					isNaN(k) || isNaN(g) || (c.commentModel.size = new Blockly.utils.Size(k, g));
				...
				case "statement":
					...
					if (f)
						...
						if (f = Blockly.Xml.domToBlockHeadless_(f, b), f.outputConnection) g.connection.connect(f.outputConnection);
...
Blockly.InsertionMarkerManager.prototype.createMarkerBlock_ = function(a) {
	...
	try {
		...
		for (b = 0; b < a.inputList.length; b++) {
			...
			for (d = 0; d < e.fieldRow.length; d++) f.fieldRow[d].setValue(e.fieldRow[d].getValue())
...
Blockly.BlockDragger.prototype.startBlockDrag = function(a, b) {
	...
	if (this.draggingBlock_.getParent() || b && this.draggingBlock_.nextConnection && this.draggingBlock_.nextConnection.targetBlock()) {
		...
		c = Blockly.utils.Coordinate.sum(this.startXY_,
...
Blockly.Cursor.prototype.setDrawer = function(a) {
...
Blockly.Cursor.prototype.setCurNode = function(a) {
...
Blockly.ThemeManager = function(a) {
...
Blockly.Workspace.prototype.setCursor = function(a) {

	this.cursor_ = a

};

Blockly.Workspace.prototype.setMarker = function(a) {
...
Blockly.Workspace.prototype.setTheme = function(a) {
...
Blockly.Workspace.prototype.updateBlockStyles_ = function(a) {
...
Blockly.Workspace.prototype.getBlocksByType = function(a, b) {
	...
	b && 1 < c.length && (this.sortObjects_.offset = Math.sin(Blockly.utils.math.toRadians(Blockly.Workspace.SCAN_ANGLE)), this.RTL && (this.sortObjects_.offset *= -1), c.sort(this.sortObjects_));
...
Blockly.Bubble = function(a, b, c, d, e, f) {
	...
	a.options.readOnly ||
		...
		(Blockly.bindEventWithChecks_(this.bubbleBack_, "mousedown", this, this.bubbleMouseDown_), this.resizeGroup_ && Blockly.bindEventWithChecks_(this.resizeGroup_, "mousedown", this, this.resizeMouseDown_))
...
Blockly.Gesture.prototype.handleWsStart = function(a, b) {
	...
	Blockly.keyboardAccessibilityMode && Blockly.navigation.setState(Blockly.navigation.STATE_WS)
...
Blockly.Gesture.prototype.doWorkspaceClick_ = function(a) {
	...
	a.shiftKey ? (Blockly.navigation.enableKeyboardAccessibility(), a = new Blockly.utils.Coordinate(a.clientX, a.clientY), a = Blockly.utils.screenToWsCoordinates(b, a), a = Blockly.ASTNode.createWorkspaceNode(b, a), b.getCursor().setCurNode(a)) : Blockly.selected && Blockly.selected.unselect()
...
Blockly.Field.prototype.createBorderRect_ = function() {
	...
	this.size_.height = Math.max(this.size_.height, Blockly.Field.BORDER_RECT_DEFAULT_HEIGHT);

	this.size_.width = Math.max(this.size_.width, Blockly.Field.X_PADDING);
...
Blockly.Field.prototype.createTextElement_ = function() {
	...
	this.textElement_ = Blockly.utils.dom.createSvgElement("text", {
		...
		x: this.borderRect_ ? Blockly.Field.DEFAULT_TEXT_OFFSET : 0
...
Blockly.Field.prototype.updateSize_ = function() {
	...
	var a = Blockly.utils.dom.getTextWidth(this.textElement_);

	this.borderRect_ && (a += Blockly.Field.X_PADDING, this.borderRect_.setAttribute("width", a));
...
Blockly.Field.prototype.onBlocklyAction = function(a) {
...
Blockly.fieldRegistry.unregister = function(a) {
	...
	Blockly.fieldRegistry.typeMap_[a] ? delete Blockly.fieldRegistry.typeMap_[a] : console.warn('No field mapping for type "' + a + '" found to unregister')
...
Blockly.Block = function(a, b, c) {
	...
	this.setDataStr = function(obj, bool){
		...
		if(!obj || !bool){
			throw 'Invalid call to setDataStr()';
		}
	...
	this.getDataStr = function(){
		...
		if(this.dataStr.isVar){
			return 'isVar';
		}
		
		if(this.dataStr.isArr){
			return 'isArr';
		}
		
		if(this.dataStr.isVec){
			return 'isVec';
		}
		...
		if(this.dataStr.isStruct){
			return 'isStruct';
		}
		
		if(this.dataStr.isClass){
			return 'isClass';
		}
...
Blockly.Block.prototype.dispose = function(a) {
	...
	if (this.workspace) {
		...
		Blockly.keyboardAccessibilityMode && Blockly.navigation.moveCursorOnBlockDelete(this);
...
Blockly.Block.prototype.getColourShadow = function() {
	...
	return a ? a : Blockly.utils.colour.blend("#fff", this.getColour(), .6)
...
Blockly.Block.prototype.getColourBorder = function() {
	...
	return {
		...
		colourLight: Blockly.utils.colour.blend("#fff", a, .3),

		colourDark: Blockly.utils.colour.blend("#000", a, .2)
...
Blockly.Block.prototype.setColour = function(a) {
	...
	if (!isNaN(c) && 0 <= c && 360 >= c) this.hue_ = c, this.colour_ = Blockly.hueToHex(c);

	else if (c = Blockly.utils.colour.parse(b)) this.colour_ = c, this.hue_ = null;
			...
			do
				...
				if (h = !1, "string" == typeof e) f = new Blockly.FieldLabel(e);
					...
					default:
						...
						f = Blockly.fieldRegistry.fromJson(e), !f && e.alt && (e = e.alt, h = !0)
...
Blockly.Block.prototype.setCommentText = function(a) {
	...
	this.commentModel.text != a && (Blockly.Events.fire(new Blockly.Events.BlockChange(this, "comment", null, this.commentModel.text, a)), this.comment = this.commentModel.text = a)
...
Blockly.blockRendering.IPathObject = function(a) {};

Blockly.blockRendering.PathObject = function(a) {
...
Blockly.blockRendering.PathObject.prototype.setPaths = function(a) {
...
Blockly.Component.prototype.setElementInternal = function(a) {
...
Blockly.Component.prototype.render = function(a) {

	this.render_(a)

};

Blockly.Component.prototype.renderBefore = function(a) {
...
Blockly.Component.prototype.addChild = function(a, b) {
...
Blockly.Component.prototype.getChild = function(a) {

	return a ? this.childIndex_[a] || null : null

};

Blockly.Component.prototype.getChildAt = function(a) {

	return this.children_[a] || null

};

Blockly.Component.prototype.forEachChild = function(a, b) {

	for (var c = 0; c < this.children_.length; c++) a.call(b, this.children_[c], c)

};

Blockly.Component.prototype.indexOfChild = function(a) {
...
Blockly.utils.aria.setRole = function(a, b) {
	...
	b ? a.setAttribute(Blockly.utils.aria.ROLE_ATTRIBUTE_, b) : Blockly.utils.aria.removeRole(a)

};

Blockly.utils.aria.getRole = function(a) {

	return a.getAttribute(Blockly.utils.aria.ROLE_ATTRIBUTE_) || null

};

Blockly.utils.aria.removeRole = function(a) {

	a.removeAttribute(Blockly.utils.aria.ROLE_ATTRIBUTE_)
...
Blockly.utils.aria.setState = function(a, b, c) {
	...
	b = Blockly.utils.aria.getAriaAttributeName_(b);

	a.setAttribute(b, c)

};

Blockly.utils.aria.getAriaAttributeName_ = function(a) {

	return Blockly.utils.aria.ARIA_PREFIX_ + a
...
Blockly.Menu.prototype.focus = function() {
	...
	a && (a.focus(), Blockly.utils.dom.addClass(a, "focused"))
...
Blockly.Menu.prototype.canHighlightItem = function(a) {
...
Blockly.Menu.prototype.handleKeyEvent = function(a) {
...
Blockly.MenuItem.prototype.createDom = function() {
	...
	Blockly.utils.aria.setRole(a, this.roleName_ || (this.checkable_ ?

		Blockly.utils.aria.Role.MENUITEMCHECKBOX : Blockly.utils.aria.Role.MENUITEM));
...
Blockly.MenuItem.prototype.setContentInternal = function(a) {

	this.content_ = a

};

Blockly.MenuItem.prototype.setValue = function(a) {
...
Blockly.MenuItem.prototype.handleClick = function(a) {
...
Blockly.RenderedConnection.prototype.highlight = function() {
	...
	a = this.type == Blockly.INPUT_VALUE || this.type == Blockly.OUTPUT_VALUE ? Blockly.utils.svgPaths.moveBy(0, -5) + Blockly.utils.svgPaths.lineOnAxis("v", 5) + a.PUZZLE_TAB.pathDown + Blockly.utils.svgPaths.lineOnAxis("v", 5) : Blockly.utils.svgPaths.moveBy(-5, 0) + Blockly.utils.svgPaths.lineOnAxis("h", 5) + a.NOTCH.pathLeft + Blockly.utils.svgPaths.lineOnAxis("h", 5);
...
Blockly.Warning.prototype.setVisible = function(a) {
	...
	a != this.isVisible() && (Blockly.Events.fire(new Blockly.Events.Ui(this.block_, "warningOpen", !a, a)), a ? this.createBubble() : this.disposeBubble())
...
Blockly.Warning.prototype.createBubble = function() {
	...
	this.bubble_ = new Blockly.Bubble(this.block_.workspace, this.paragraphElement_, this.block_.svgPath_, this.iconXY_, null, null);
	...
	if (!this.isInFlyout) {
		...
		if (this.isMovable())
			...
			if (this.collapsed_) this.workspace.options.collapse && (c = {
				...
				}, c.text = Blockly.Msg.EXPAND_BLOCK, c.callback =
...
Blockly.BlockSvg.prototype.setConnectionsHidden = function(a) {
...
Blockly.BlockSvg.prototype.positionNearConnection = function(a, b) {
	...
	a.type != Blockly.NEXT_STATEMENT && a.type != Blockly.INPUT_VALUE || this.moveBy(b.x_ - a.x_, b.y_ - a.y_)
...
Blockly.DropDownDiv.createDom = function() {
	...
	if (!Blockly.DropDownDiv.DIV_) {
		...
		a.style.backgroundColor = Blockly.DropDownDiv.DEFAULT_DROPDOWN_COLOR;

		a.style.borderColor = Blockly.DropDownDiv.DEFAULT_DROPDOWN_BORDER_COLOR;
...
Blockly.DropDownDiv.showPositionedByBlock = function(a, b, c, d) {
	...
	return Blockly.DropDownDiv.show(a, b.RTL, f, g, f, e, c)
...
Blockly.DropDownDiv.showPositionedByField = function(a, b, c) {
	...
	Blockly.DropDownDiv.setBoundsElement(c.workspace.getParentSvg().parentNode);

	return Blockly.DropDownDiv.show(a, c.RTL, e, f, e, d, b)
...
Blockly.DropDownDiv.show = function(a, b, c, d, e, f, g) {
	...
	a = Blockly.DropDownDiv.getPositionMetrics(c, d, e, f);

	a.arrowVisible ? (Blockly.DropDownDiv.arrow_.style.display = "", Blockly.DropDownDiv.arrow_.style.transform = "translate(" + a.arrowX + "px," + a.arrowY + "px) rotate(45deg)", Blockly.DropDownDiv.arrow_.setAttribute("class", a.arrowAtTop ? "blocklyDropDownArrow arrowTop" : "blocklyDropDownArrow arrowBottom")) : Blockly.DropDownDiv.arrow_.style.display = "none";

	Blockly.DropDownDiv.DIV_.style.direction = b ? "rtl" : "ltr";
...
Blockly.DropDownDiv.getPositionMetrics = function(a, b, c, d) {
	...
	return b + f.height < e.bottom ? Blockly.DropDownDiv.getPositionBelowMetrics(a, b, e, f) : d - f.height > e.top ? Blockly.DropDownDiv.getPositionAboveMetrics(c, d, e, f) : b + f.height < document.documentElement.clientHeight ? Blockly.DropDownDiv.getPositionBelowMetrics(a, b, e, f) : d - f.height > document.documentElement.clientTop ? Blockly.DropDownDiv.getPositionAboveMetrics(c, d,

		e, f) : Blockly.DropDownDiv.getPositionTopOfPageMetrics(a, e, f)

};

Blockly.DropDownDiv.getPositionBelowMetrics = function(a, b, c, d) {
...
Blockly.DropDownDiv.getPositionAboveMetrics = function(a, b, c, d) {
...
Blockly.DropDownDiv.getPositionTopOfPageMetrics = function(a, b, c) {
...
Blockly.DropDownDiv.hideWithoutAnimation = function() {
	...
	if (Blockly.DropDownDiv.isVisible()) {
		...
		Blockly.DropDownDiv.onHide_ && (Blockly.DropDownDiv.onHide_(), Blockly.DropDownDiv.onHide_ =
...
Blockly.Options = function(a) {
	...
	p = n ? p ? Blockly.TOOLBOX_AT_TOP : Blockly.TOOLBOX_AT_BOTTOM : p == m ? Blockly.TOOLBOX_AT_RIGHT : Blockly.TOOLBOX_AT_LEFT;
	...
	var t = void 0 === a.oneBasedIndex ? !0 : !!a.oneBasedIndex,
		...
		v = a.keyMap || Blockly.user.keyMap.createDefaultKeyMap(),
...
Blockly.blockRendering.unregister = function(a) {
	...
	Blockly.blockRendering.rendererMap_[a] ? delete Blockly.blockRendering.rendererMap_[a] : console.warn('No renderer mapping for name "' + a + '" found to unregister')
...
Blockly.blockRendering.init = function(a) {
	...
	Blockly.utils.object.inherits(b, Blockly.blockRendering.rendererMap_[a]);
...
Blockly.WorkspaceSvg.prototype.setCursor = function(a) {
...
Blockly.WorkspaceSvg.prototype.setMarker = function(a) {
...
Blockly.WorkspaceSvg.prototype.pasteBlock_ = function(a) {
	...
	try {
		...
		if (Blockly.keyboardAccessibilityMode && c) {

			Blockly.navigation.insertBlock(b, c.getLocation());
			...
			do {
				...
				if (!a) {
					...
					for (var l; l = k[c]; c++)
						...
						if (l.closest(Blockly.SNAP_RADIUS, new Blockly.utils.Coordinate(d, e)).connection) {
				...
				a && (d = this.RTL ? d - Blockly.SNAP_RADIUS : d + Blockly.SNAP_RADIUS, e += 2 * Blockly.SNAP_RADIUS)
...
Blockly.inject = function(a, b) {
	...
	e = Blockly.createMainWorkspace_(e, c,
		...
		e.addChangeListener(function(a) {
			...
			if (!e.isDragging() && !e.isMovable() && -1 != Blockly.Events.BUMP_EVENTS.indexOf(a.type)) {
				...
				if (b.contentTop < b.viewTop || b.contentBottom > b.viewBottom || b.contentLeft < b.viewLeft || b.contentRight > b.viewRight) {
					...
					a && (a.group || console.log("WARNING: Moved object in bounds but there was no event group. This may break undo."), null !== c && Blockly.Events.setGroup(c))
...
Blockly.inject.bindDocumentEvents_ = function() {
	...
	}), Blockly.bindEventWithChecks_(document, "keydown", null, Blockly.onKeyDown_), Blockly.bindEvent_(document, "touchend", null, Blockly.longStop_), Blockly.bindEvent_(document, "touchcancel", null, Blockly.longStop_), Blockly.utils.userAgent.IPAD && Blockly.bindEventWithChecks_(window,
...
Blockly.ASTNode.createFieldNode = function(a) {
	...
	return new Blockly.ASTNode(Blockly.ASTNode.types.FIELD, a)

};

Blockly.ASTNode.createConnectionNode = function(a) {

	return a ? a.type == Blockly.INPUT_VALUE || a.type == Blockly.NEXT_STATEMENT && a.getParentInput() ? Blockly.ASTNode.createInputNode(a.getParentInput()) : a.type == Blockly.NEXT_STATEMENT ? new Blockly.ASTNode(Blockly.ASTNode.types.NEXT, a) : a.type == Blockly.OUTPUT_VALUE ? new Blockly.ASTNode(Blockly.ASTNode.types.OUTPUT, a) : a.type == Blockly.PREVIOUS_STATEMENT ? new Blockly.ASTNode(Blockly.ASTNode.types.PREVIOUS, a) : null : null

};

Blockly.ASTNode.createInputNode = function(a) {

	return a ? new Blockly.ASTNode(Blockly.ASTNode.types.INPUT, a.connection) : null

};

Blockly.ASTNode.createBlockNode = function(a) {

	return new Blockly.ASTNode(Blockly.ASTNode.types.BLOCK, a)

};

Blockly.ASTNode.createStackNode = function(a) {

	return new Blockly.ASTNode(Blockly.ASTNode.types.STACK, a)

};

Blockly.ASTNode.createWorkspaceNode = function(a, b) {

	return new Blockly.ASTNode(Blockly.ASTNode.types.WORKSPACE, a, {
...
Blockly.ASTNode.prototype.findPreviousEditableField_ = function(a, b, c) {
	...
	for (c = (c ? b.length : a) - 1; a = b[c]; c--)
		...
		if (a.EDITABLE) return b = a, Blockly.ASTNode.createFieldNode(b);
...
Blockly.ASTNode.prototype.findNextForInput_ = function() {
	...
	for (var c; c = b.inputList[a]; a++) {
		...
		for (var d = c.fieldRow, e = 0, f; f = d[e]; e++)
			...
			if (f.EDITABLE) return Blockly.ASTNode.createFieldNode(f);
...
Blockly.ASTNode.prototype.findNextForField_ = function() {
	...
	for (a = b.fieldRow.indexOf(a) + 1; b = c.inputList[d]; d++) {
		...
		for (var e = b.fieldRow; a < e.length;) {
			...
			if (e[a].EDITABLE) return Blockly.ASTNode.createFieldNode(e[a]);
...
Blockly.ASTNode.prototype.next = function() {
	...
	switch (this.type_) {
		...
		case Blockly.ASTNode.types.BLOCK:
			...
			if (a) return Blockly.ASTNode.createConnectionNode(a);
		...
		case Blockly.ASTNode.types.NEXT:
			...
			if (a = this.location_.targetConnection) return Blockly.ASTNode.createConnectionNode(a)
...
Blockly.ASTNode.prototype.prev = function() {
	...
	switch (this.type_) {
		...
		case Blockly.ASTNode.types.BLOCK:
			...
			if (a = this.location_.previousConnection || a) return Blockly.ASTNode.createConnectionNode(a);
		...
		case Blockly.ASTNode.types.PREVIOUS:
			...
			if ((a = this.location_.targetConnection) &&
				...
				!a.getParentInput()) return Blockly.ASTNode.createConnectionNode(a);
...
Blockly.user.keyMap.setKeyMap = function(a) {

	Blockly.user.keyMap.map_ = a
...
Blockly.user.keyMap.getActionByKeyCode = function(a) {

	return Blockly.user.keyMap.map_[a]
...
Blockly.user.keyMap.serializeKeyEvent = function(a) {

	for (var b = Blockly.utils.object.values(Blockly.user.keyMap.modifierKeys), c = "", d = 0, e; e = b[d]; d++) a.getModifierState(e) && (c += e);
...
Blockly.user.keyMap.createSerializedKey = function(a, b) {
	...
	for (var c = "", d = Blockly.utils.object.values(Blockly.user.keyMap.modifierKeys), e = 0, f; f = b[e]; e++)
...
Blockly.user.keyMap.createDefaultKeyMap = function() {
	...
	var a = {},
		...
		b = Blockly.user.keyMap.createSerializedKey(Blockly.utils.KeyCodes.K, [Blockly.user.keyMap.modifierKeys.CONTROL]);
...
Blockly.navigation.focusToolbox_ = function() {
	...
	b && (Blockly.navigation.currentState_ = Blockly.navigation.STATE_TOOLBOX, Blockly.navigation.resetFlyout_(!1), a.getMarker().getCurNode() || Blockly.navigation.markAtCursor_(), b.selectFirstCategory())
...
Blockly.navigation.focusFlyout_ = function() {
	...
	a.getMarker().getCurNode() || Blockly.navigation.markAtCursor_();

	b && b.getWorkspace() && (a = b.getWorkspace().getTopBlocks(!0), 0 < a.length && (a = a[0], a = Blockly.ASTNode.createStackNode(a), Blockly.navigation.getFlyoutCursor_().setCurNode(a)))
...
Blockly.navigation.insertFromFlyout = function() {
	...
	if (b && b.isVisible()) {
		...
		c.isEnabled() ? (b = b.createBlock(c), b.render(), b.setConnectionsHidden(!1), a.getCursor().setCurNode(Blockly.ASTNode.createBlockNode(b)), Blockly.navigation.modify_() || Blockly.navigation.warn_("Something went wrong while inserting a block from the flyout."), Blockly.navigation.focusWorkspace_(), a.getCursor().setCurNode(Blockly.navigation.getTopNode(b)),
...
Blockly.navigation.resetFlyout_ = function(a) {

	Blockly.navigation.getFlyoutCursor_() && (Blockly.navigation.getFlyoutCursor_().hide(), a && Blockly.getMainWorkspace().getFlyout().hide())

};

Blockly.navigation.modifyWarn_ = function() {

	var a = Blockly.getMainWorkspace().getMarker().getCurNode(),
	...
	return a == Blockly.ASTNode.types.FIELD ? (Blockly.navigation.warn_("Should not have been able to mark a field."), !1) : a == Blockly.ASTNode.types.BLOCK ? (Blockly.navigation.warn_("Should not have been able to mark a block."),
...
Blockly.navigation.modify_ = function() {
	...
	if (c == Blockly.ASTNode.types.WORKSPACE) return b =
		...
		Blockly.navigation.getSourceBlock_(b), Blockly.navigation.moveBlockToWorkspace_(b, a);
...
Blockly.navigation.moveAndConnect_ = function(a, b) {

	if (!a || !b) return !1;

	var c = a.getSourceBlock();

	return b.canConnectWithReason_(a) == Blockly.Connection.CAN_CONNECT ? (Blockly.navigation.disconnectChild_(a, b), b.isSuperior() || c.getRootBlock().positionNearConnection(a, b), b.connect(a), !0) : !1

};

Blockly.navigation.getInferiorConnection_ = function(a) {
...
Blockly.navigation.getSuperiorConnection_ = function(a) {
...
Blockly.navigation.connect_ = function(a, b) {
	...
	try {
		...
		Blockly.navigation.warn_("Connection failed with error: " + g)
...
Blockly.navigation.setState = function(a) {

	Blockly.navigation.currentState_ = a

};

Blockly.navigation.getSourceBlock_ = function(a) {

	return a ? a.getType() === Blockly.ASTNode.types.BLOCK ? a.getLocation() : a.getType() === Blockly.ASTNode.types.STACK ? a.getLocation() : a.getType() === Blockly.ASTNode.types.WORKSPACE ? null : a.getLocation().getSourceBlock() : null

};

Blockly.navigation.getTopNode = function(a) {
...
Blockly.navigation.moveCursorOnBlockDelete = function(a) {
	...
	if (b && (b = b.getCursor())) {
		...
		c = Blockly.navigation.getSourceBlock_(c);
...
Blockly.navigation.enableKeyboardAccessibility = function() {
	...
	Blockly.keyboardAccessibilityMode || (Blockly.keyboardAccessibilityMode = !0, Blockly.navigation.focusWorkspace_())
...
Blockly.navigation.disableKeyboardAccessibility = function() {
	...
	if (Blockly.keyboardAccessibilityMode) {
		...
		Blockly.keyboardAccessibilityMode = !1;
...
Blockly.navigation.log_ = function(a) {

	Blockly.navigation.loggingCallback ? Blockly.navigation.loggingCallback("log", a) : console.log(a)

};

Blockly.navigation.warn_ = function(a) {

	Blockly.navigation.loggingCallback ? Blockly.navigation.loggingCallback("warn", a) : console.warn(a)

};

Blockly.navigation.error_ = function(a) {

	Blockly.navigation.loggingCallback ? Blockly.navigation.loggingCallback("error", a) : console.error(a)

};

Blockly.navigation.onKeyPress = function(a) {

	a = Blockly.user.keyMap.serializeKeyEvent(a);

	return (a = Blockly.user.keyMap.getActionByKeyCode(a)) ? Blockly.navigation.onBlocklyAction(a) : !1
...
Blockly.navigation.onBlocklyAction = function(a) {
	...
	Blockly.keyboardAccessibilityMode ? b ? -1 < Blockly.navigation.READONLY_ACTION_LIST.indexOf(a) && (c = Blockly.navigation.handleActions_(a)) : c = Blockly.navigation.handleActions_(a) : a.name === Blockly.navigation.actionNames.TOGGLE_KEYBOARD_NAV && (Blockly.navigation.enableKeyboardAccessibility(), c = !0);
...
Blockly.navigation.handleActions_ = function(a) {
	...
	if (a.name === Blockly.navigation.actionNames.TOGGLE_KEYBOARD_NAV) return Blockly.navigation.disableKeyboardAccessibility(), !0;

	if (a.name === Blockly.navigation.actionNames.TOOLBOX) return b.getToolbox() ? Blockly.navigation.focusToolbox_() : Blockly.navigation.focusFlyout_(), !0;
	...
	if (Blockly.navigation.currentState_ === Blockly.navigation.STATE_WS) {
		...
		b && b.getType() === Blockly.ASTNode.types.FIELD && (c =

			b.getLocation().onBlocklyAction(a));

		c || (c = Blockly.navigation.workspaceOnAction_(a));

		return c

	}

	return Blockly.navigation.currentState_ === Blockly.navigation.STATE_FLYOUT ? Blockly.navigation.flyoutOnAction_(a) : Blockly.navigation.currentState_ === Blockly.navigation.STATE_TOOLBOX ? Blockly.navigation.toolboxOnAction_(a) : !1
...
Blockly.navigation.workspaceOnAction_ = function(a) {
	...
	switch (a.name) {
		...
		case Blockly.navigation.actionNames.MARK:
			...
			return Blockly.navigation.handleEnterForWS_(),
...
Blockly.navigation.flyoutOnAction_ = function(a) {
	...
	switch (a.name) {
		...
		case Blockly.navigation.actionNames.PREVIOUS:
			...
			return Blockly.navigation.getFlyoutCursor_().prev(), !0;
		...
		case Blockly.navigation.actionNames.NEXT:
			...
			return Blockly.navigation.getFlyoutCursor_().next(), !0;
		...
		case Blockly.navigation.actionNames.EXIT:
			...
			return Blockly.navigation.focusWorkspace_(),
...
Blockly.navigation.toolboxOnAction_ = function(a) {
	...
	if (a.name === Blockly.navigation.actionNames.EXIT) return Blockly.navigation.focusWorkspace_(), !0;

	var b = Blockly.getMainWorkspace().getToolbox().onBlocklyAction(a);

	return b || a.name !== Blockly.navigation.actionNames.IN ? b : (Blockly.navigation.focusFlyout_(), !0)
...
Blockly.navigation.handleEnterForWS_ = function() {
	...
	b == Blockly.ASTNode.types.FIELD ? a.getLocation().showEditor_() : a.isConnection() || b == Blockly.ASTNode.types.WORKSPACE ? Blockly.navigation.markAtCursor_() : b == Blockly.ASTNode.types.BLOCK ? Blockly.navigation.warn_("Cannot mark a block.") : b == Blockly.ASTNode.types.STACK && Blockly.navigation.warn_("Cannot mark a stack.")
...
Blockly.navigation.ACTION_DISCONNECT = new Blockly.Action(Blockly.navigation.actionNames.DISCONNECT, "Dicsonnect the block at thecurrent location from its parent.");
			...
			Blockly.Variables.promptName(Blockly.Msg.NEW_VARIABLE_TITLE, c, function(c) {
				...
				if (c) {
					...
					if (f) {
						...
						if (f.type == d) var l = Blockly.Msg.VARIABLE_ALREADY_EXISTS.replace("%1", g);

						else l = Blockly.Msg.VARIABLE_ALREADY_EXISTS_FOR_ANOTHER_TYPE, l = l.replace("%1", g).replace("%2", f.type);
...
Blockly.WidgetDiv.show = function(a, b, c) {
	...
	a = Blockly.utils.style.getViewportPageOffset();
...
Blockly.onKeyDown_ = function(a) {
	...
	if (!(Blockly.utils.isTargetInput(a) || b.rendered && !b.isVisible()))
...
Blockly.hideChaff = function(a) {
	...
	a || (a = Blockly.getMainWorkspace(), a.trashcan && a.trashcan.flyout_ && a.trashcan.flyout_.hide(), a.toolbox_ && a.toolbox_.flyout_ && a.toolbox_.flyout_.autoClose && a.toolbox_.clearSelection())
...
Blockly.Comment.prototype.createEditor_ = function() {
	...
	Blockly.bindEventWithChecks_(b, "mouseup", this, this.startEdit_, !0, !0);
	...
	Blockly.bindEventWithChecks_(b, "change", this, function(a) {
		...
		this.cachedText_ != this.model_.text && Blockly.Events.fire(new Blockly.Events.BlockChange(this.block_, "comment", null, this.cachedText_, this.model_.text))

	});

	Blockly.bindEventWithChecks_(b, "input",
...
Blockly.Comment.prototype.setVisible = function(a) {
	...
	a != this.isVisible() && (Blockly.Events.fire(new Blockly.Events.Ui(this.block_, "commentOpen", !a, a)), (this.model_.pinned = a) ? this.createBubble_() : this.disposeBubble_())
...
Blockly.Comment.prototype.createEditableBubble_ = function() {
	...
	this.bubble_ = new Blockly.Bubble(this.block_.workspace, this.createEditor_(), this.block_.svgPath_, this.iconXY_, this.model_.size.width, this.model_.size.height);
...
Blockly.Comment.prototype.disposeBubble_ = function() {
	...
	this.paragraphElement_ ? Blockly.Warning.prototype.disposeBubble.call(this) : (this.bubble_.dispose(), this.foreignObject_ = this.textarea_ = this.bubble_ = null)
...
Blockly.tree.BaseNode.prototype.addChildAt = function(a, b) {
	...
	if (e = this.getElement())
		...
		if (this.updateExpandIcon(), Blockly.utils.aria.setState(e, Blockly.utils.aria.State.EXPANDED, this.getExpanded()), this.getExpanded()) {
			...
			d || (c ? c.updateExpandIcon() : (Blockly.utils.style.setElementShown(e, !0), this.setExpanded(this.getExpanded())))
...
Blockly.tree.BaseNode.prototype.setSelectedInternal = function(a) {
...
Blockly.tree.BaseNode.prototype.setExpandedInternal = function(a) {
...
Blockly.tree.BaseNode.prototype.setExpanded = function(a) {
	...
	if (this.hasChildren()) {
		...
		if (!a && d && this.contains(d.getSelectedItem()) && this.select(), e) {
			...
			if (c = this.getChildrenElement()) Blockly.utils.style.setElementShown(c, a), Blockly.utils.aria.setState(e, Blockly.utils.aria.State.EXPANDED, a), a && this.isInDocument() && !c.hasChildNodes() && (this.forEachChild(function(a) {
...
Blockly.tree.BaseNode.prototype.setText = function(a) {
...
Blockly.tree.BaseNode.prototype.setTreeInternal = function(a) {
...
Blockly.tree.TreeNode.prototype.onKeyDown = function(a) {
	...
	if (this.tree.toolbox_.horizontalLayout_) {
		...
		b[Blockly.utils.KeyCodes.RIGHT] = this.isRightToLeft() ? d : c;

		b[Blockly.utils.KeyCodes.LEFT] = this.isRightToLeft() ? c : d;
...
Blockly.tree.TreeNode.prototype.onSizeChanged = function(a) {
...
Blockly.tree.TreeControl.prototype.setExpanded = function(a) {
...
Blockly.tree.TreeControl.prototype.getRowClassName = function() {
	...
	return Blockly.tree.TreeControl.superClass_.getRowClassName.call(this) + " " + this.getConfig().cssHideRoot
...
Blockly.tree.TreeControl.prototype.onBeforeSelected = function(a) {

	this.onBeforeSelected_ = a

};

Blockly.tree.TreeControl.prototype.onAfterSelected = function(a) {
...
Blockly.tree.TreeControl.prototype.handleKeyEvent_ = function(a) {

	var b = !1;

	if (b = this.selectedItem_ && this.selectedItem_.onKeyDown(a) || b) Blockly.utils.style.scrollIntoContainerView(this.selectedItem_.getElement(), this.getElement().parentNode), a.preventDefault();
...
Blockly.tree.TreeControl.prototype.createNode = function(a) {

	return new Blockly.tree.TreeNode(this.toolbox_, a || "", this.getConfig())
...
Blockly.FieldTextInput.prototype.doValueInvalid_ = function(a) {
	...
	this.isBeingEdited_ && (this.isTextValid_ = !1, a = this.value_, this.value_ = this.htmlInput_.untypedDefaultValue_, this.sourceBlock_ && Blockly.Events.isEnabled() && Blockly.Events.fire(new Blockly.Events.BlockChange(this.sourceBlock_, "field", this.name || null, a, this.value_)))
...
Blockly.FieldTextInput.prototype.render_ = function() {
	...
	this.isBeingEdited_ && (this.sourceBlock_.RTL ? setTimeout(this.resizeEditor_.bind(this), 0) : this.resizeEditor_(), this.isTextValid_ ? (Blockly.utils.dom.removeClass(this.htmlInput_, "blocklyInvalidInput"), Blockly.utils.aria.setState(this.htmlInput_, "invalid", !1)) : (Blockly.utils.dom.addClass(this.htmlInput_, "blocklyInvalidInput"), Blockly.utils.aria.setState(this.htmlInput_, "invalid", !0)))
...
Blockly.FieldTextInput.prototype.showPromptEditor_ = function() {
	...
	Blockly.prompt(Blockly.Msg.CHANGE_VALUE_TITLE, this.getText(), function(b) {
...
Blockly.FieldTextInput.prototype.widgetCreate_ = function() {
	...
	b.style.borderRadius = Blockly.FieldTextInput.BORDERRADIUS * this.workspace_.scale + "px";
...
Blockly.FieldTextInput.prototype.onHtmlInputChange_ = function(a) {
	...
	a !== this.htmlInput_.oldValue_ && (this.htmlInput_.oldValue_ = a, Blockly.Events.setGroup(!0), a = this.getValueFromEditorText_(a), this.setValue(a), this.forceRerender(), Blockly.Events.setGroup(!1))
...
Blockly.FieldAngle.prototype.dropdownCreate_ = function() {
	...
	this.clickSurfaceWrapper_ = Blockly.bindEventWithChecks_(b, "click", this, this.onMouseMove, !0, !0);

	this.moveSurfaceWrapper_ = Blockly.bindEventWithChecks_(b, "mousemove", this, this.onMouseMove, !0, !0);
...
Blockly.FieldCheckbox.prototype.initView = function() {
	...
	this.textContent_.nodeValue = this.checkChar_ || Blockly.FieldCheckbox.CHECK_CHAR;
...
Blockly.FieldCheckbox.prototype.setCheckCharacter = function(a) {
	...
	this.textContent_ && (this.textContent_.nodeValue = a || Blockly.FieldCheckbox.CHECK_CHAR)
...
Blockly.FieldColour = function(a, b, c) {
	...
	this.size_ = new Blockly.utils.Size(Blockly.FieldColour.DEFAULT_WIDTH, Blockly.FieldColour.DEFAULT_HEIGHT)
...
Blockly.FieldColour.prototype.onBlocklyAction = function(a) {

	if (this.picker_) {

		if (a === Blockly.navigation.ACTION_PREVIOUS) return this.moveHighlightBy_(0, -1), !0;

		if (a === Blockly.navigation.ACTION_NEXT) return this.moveHighlightBy_(0, 1), !0;

		if (a === Blockly.navigation.ACTION_OUT) return this.moveHighlightBy_(-1, 0), !0;

		if (a === Blockly.navigation.ACTION_IN) return this.moveHighlightBy_(1, 0), !0

	}

	return Blockly.FieldColour.superClass_.onBlocklyAction.call(this, a)
...
Blockly.FieldColour.prototype.dropdownCreate_ = function() {
	...
	Blockly.utils.aria.setState(e, "rowcount", Math.floor(b.length / a));

	Blockly.utils.aria.setState(e,
	...
	for (var f, g = 0; g < b.length; g++) {
		...
		Blockly.utils.aria.setState(h, Blockly.utils.aria.State.SELECTED,
	...
	this.onMouseLeaveWrapper_ = Blockly.bindEventWithChecks_(e, "mouseleave", this, this.onMouseLeave_, !0);

	this.onKeyDownWrapper_ =

		Blockly.bindEventWithChecks_(e, "keydown", this, this.onKeyDown_);
...
Blockly.FieldDropdown = function(a, b, c) {
	...
	Blockly.FieldDropdown.superClass_.constructor.call(this, a[1], b, c);
...
Blockly.FieldDropdown.prototype.renderSelectedImage_ = function(a) {
	...
	var b = Blockly.utils.dom.getTextWidth(this.arrow_),

		c = Number(a.width);

	this.size_.height = Number(a.height) + Blockly.FieldDropdown.IMAGE_Y_PADDING;

	this.size_.width = c + b + Blockly.Field.X_PADDING;

	this.sourceBlock_.RTL ? (a = Blockly.Field.DEFAULT_TEXT_OFFSET -
...
Blockly.FieldDropdown.prototype.renderSelectedText_ = function() {
	...
	this.size_.width = Blockly.utils.dom.getTextWidth(this.textElement_) + Blockly.Field.X_PADDING
...
Blockly.FieldDropdown.prototype.onBlocklyAction = function(a) {

	if (this.menu_) {

		if (a === Blockly.navigation.ACTION_PREVIOUS) return this.menu_.highlightPrevious(), !0;

		if (a === Blockly.navigation.ACTION_NEXT) return this.menu_.highlightNext(), !0

	}

	return Blockly.FieldDropdown.superClass_.onBlocklyAction.call(this, a)
...
Blockly.FieldMultilineInput.prototype.render_ = function() {
	...
	for (var b = Blockly.Field.Y_PADDING / 2, c = 0, d = 0; d < a.length; d++) Blockly.utils.dom.createSvgElement("text", {
	...
	}, this.textGroup_).appendChild(document.createTextNode(a[d])), c += Blockly.FieldMultilineInput.LINE_HEIGHT;
...
Blockly.FieldMultilineInput.prototype.updateSize_ = function() {
	...
	for (var a = this.textGroup_.childNodes, b = 0, c = 0, d = 0; d < a.length; d++) {
		...
		c += Blockly.FieldMultilineInput.LINE_HEIGHT

	}

	this.borderRect_ && (b += Blockly.Field.X_PADDING, this.borderRect_.setAttribute("width", b), this.borderRect_.setAttribute("height", c));
...
Blockly.FieldMultilineInput.prototype.widgetCreate_ = function() {
	...
	var d = Blockly.FieldTextInput.FONTSIZE * b + "pt";
	...
	d = Blockly.Field.DEFAULT_TEXT_OFFSET * b;

	c.style.paddingLeft = d + "px";

	c.style.width = "calc(100% - " + d + "px)";

	c.style.lineHeight = Blockly.FieldMultilineInput.LINE_HEIGHT * b + "px";
...
Blockly.FlyoutButton.prototype.createDom = function() {
	...
	this.width = Blockly.utils.dom.getTextWidth(c);

	this.height = 20;

	this.isLabel_ || (this.width += 2 * Blockly.FlyoutButton.MARGIN, b.setAttribute("width",
...
Blockly.Css.register(".blocklyFlyoutButton {,fill: #888;,cursor: default;,},.blocklyFlyoutButtonShadow {,fill: #666;,},.blocklyFlyoutButton:hover {,fill: #aaa;,},.blocklyFlyoutLabel {,cursor: default;,},.blocklyFlyoutLabelBackground {,opacity: 0;,},.blocklyFlyoutLabelText {,fill: #000;,}".split(","));
...
Blockly.VerticalFlyout.prototype.getClientRect = function() {
	...
	return new Blockly.utils.Rect(-1E9,
...
Blockly.CursorSvg = function(a, b) {
...
Blockly.CursorSvg.HEIGHT_MULTIPLIER = .75;
...
Blockly.CursorSvg.prototype.createDom = function() {
	...
	this.svgGroup_ = Blockly.utils.dom.createSvgElement("g", {
		...
		"class": this.isMarker_ ? Blockly.CursorSvg.MARKER_CLASS : Blockly.CursorSvg.CURSOR_CLASS
...
Blockly.CursorSvg.prototype.setParent_ = function(a) {
...
Blockly.CursorSvg.prototype.showWithBlockPrevOutput_ = function(a) {
	...
	if (a) {
		...
		var b = a.width,
			...
			d = c * Blockly.CursorSvg.HEIGHT_MULTIPLIER,

			e = Blockly.CursorSvg.BLOCK_PADDING;
...
Blockly.CursorSvg.prototype.showWithCoordinates_ = function(a) {
	...
	this.workspace_.RTL && (a -= Blockly.CursorSvg.CURSOR_WIDTH);

	this.positionLine_(a, b, Blockly.CursorSvg.CURSOR_WIDTH);
...
Blockly.CursorSvg.prototype.showWithField_ = function(a) {
...
Blockly.CursorSvg.prototype.showWithInput_ = function(a) {
...
Blockly.CursorSvg.prototype.showWithNext_ = function(a) {
...
Blockly.CursorSvg.prototype.showWithStack_ = function(a) {

	a = a.getLocation();

	var b = a.getHeightWidth(),

		c = b.width + Blockly.CursorSvg.STACK_PADDING;

	b = b.height + Blockly.CursorSvg.STACK_PADDING;

	var d = -1 * Blockly.CursorSvg.STACK_PADDING / 2,

		e = -1 * Blockly.CursorSvg.STACK_PADDING / 2,
...
Blockly.CursorSvg.prototype.positionBlock_ = function(a, b, c) {

	a = Blockly.utils.svgPaths.moveBy(-1 * b, c) + Blockly.utils.svgPaths.lineOnAxis("V", -1 * b) + Blockly.utils.svgPaths.lineOnAxis("H", a + 2 * b) + Blockly.utils.svgPaths.lineOnAxis("V", c);
...
Blockly.CursorSvg.prototype.positionInput_ = function(a) {

	var b = a.getOffsetInBlock().x;

	a = a.getOffsetInBlock().y;

	var c = Blockly.utils.svgPaths.moveTo(0, 0) + this.constants_.PUZZLE_TAB.pathDown;
...
Blockly.CursorSvg.prototype.positionLine_ = function(a, b, c) {
...
Blockly.CursorSvg.prototype.positionOutput_ = function(a, b) {

	var c = Blockly.utils.svgPaths.moveBy(a, 0) + Blockly.utils.svgPaths.lineOnAxis("h", -1 * (a - this.constants_.PUZZLE_TAB.width)) + Blockly.utils.svgPaths.lineOnAxis("v", this.constants_.TAB_OFFSET_FROM_TOP) + this.constants_.PUZZLE_TAB.pathDown + Blockly.utils.svgPaths.lineOnAxis("V", b) + Blockly.utils.svgPaths.lineOnAxis("H", a);
...
Blockly.CursorSvg.prototype.positionPrevious_ = function(a, b, c) {

	a = Blockly.utils.svgPaths.moveBy(-1 * b, c) + Blockly.utils.svgPaths.lineOnAxis("V", -1 * b) + Blockly.utils.svgPaths.lineOnAxis("H", this.constants_.NOTCH_OFFSET_LEFT) + this.constants_.NOTCH.pathLeft + Blockly.utils.svgPaths.lineOnAxis("H", a + 2 * b) + Blockly.utils.svgPaths.lineOnAxis("V", c);
...
Blockly.CursorSvg.prototype.positionRect_ = function(a, b, c, d) {
...
Blockly.CursorSvg.prototype.flipRtl_ = function(a) {
...
Blockly.CursorSvg.prototype.draw = function(a) {

	a ? a.getType() === Blockly.ASTNode.types.BLOCK ? (a = a.getLocation(), this.showWithBlockPrevOutput_(a)) : a.getType() === Blockly.ASTNode.types.OUTPUT ? (a = a.getLocation().getSourceBlock(), this.showWithBlockPrevOutput_(a)) : a.getLocation().type === Blockly.INPUT_VALUE ? this.showWithInput_(a) : a.getLocation().type === Blockly.NEXT_STATEMENT ? this.showWithNext_(a) : a.getType() === Blockly.ASTNode.types.PREVIOUS ? (a = a.getLocation().getSourceBlock(), this.showWithBlockPrevOutput_(a)) :

		a.getType() === Blockly.ASTNode.types.FIELD ? this.showWithField_(a) : a.getType() === Blockly.ASTNode.types.WORKSPACE ? this.showWithCoordinates_(a) : a.getType() === Blockly.ASTNode.types.STACK && this.showWithStack_(a) : this.hide()

};

Blockly.CursorSvg.prototype.createCursorSvg_ = function() {

	var a = this.isMarker_ ? Blockly.CursorSvg.MARKER_COLOR : Blockly.CursorSvg.CURSOR_COLOR;
	...
	this.isMarker_ || (Blockly.utils.dom.createSvgElement("animate", {
		...
		}, this.cursorSvgLine_), Blockly.utils.dom.createSvgElement("animate", {
...
Blockly.Mutator.prototype.workspaceChanged_ = function(a) {
	...
	if (a.type != Blockly.Events.UI && (a.type != Blockly.Events.CHANGE || "disabled" != a.element)) {
		...
		if (this.rootBlock_.workspace == this.workspace_) {
			...
			a != b && Blockly.keyboardAccessibilityMode && Blockly.navigation.moveCursorOnBlockMutation(c);
...
Blockly.blockRendering.ConstantProvider = function() {
	...
	this.STATEMENT_INPUT_PADDING_LEFT =
...
Blockly.blockRendering.ConstantProvider.prototype.makeJaggedTeeth = function() {
	...
	var a = this.JAGGED_TEETH_HEIGHT,
		...
		c = Blockly.utils.svgPaths.line([Blockly.utils.svgPaths.point(6, 3), Blockly.utils.svgPaths.point(-12, 6), Blockly.utils.svgPaths.point(6, 3)]);
...
Blockly.blockRendering.ConstantProvider.prototype.makeNotch = function() {
	...
	function a(a) {
		...
		return Blockly.utils.svgPaths.line([Blockly.utils.svgPaths.point(a * d, c), Blockly.utils.svgPaths.point(3 * a, 0), Blockly.utils.svgPaths.point(a * d, -c)])
...
Blockly.blockRendering.ConstantProvider.prototype.makeOutsideCorners = function() {
	...
	a = Blockly.utils.svgPaths.arc("a", "0 0,1", a, Blockly.utils.svgPaths.point(-a, -a));
...
Blockly.blockRendering.Types.getType = function(a) {
	...
	Blockly.blockRendering.Types.hasOwnProperty(a) || (Blockly.blockRendering.Types[a] = Blockly.blockRendering.Types.nextTypeValue_, Blockly.blockRendering.Types.nextTypeValue_ <<= 1);
...
Blockly.blockRendering.TopRow.prototype.hasLeftSquareCorner = function(a) {
	...
	var b = a.hat ? "cap" === a.hat : Blockly.BlockSvg.START_HAT,
...
Blockly.blockRendering.RenderInfo = function(a, b) {
	...
	this.topRow =
		...
		new Blockly.blockRendering.TopRow(this.constants_);
...
Blockly.blockRendering.RenderInfo.prototype.createRows_ = function() {
	...
	for (c = 0; b = this.block_.inputList[c]; c++)
		...
		if (b.isVisible()) {
			...
			this.shouldStartNewRow_(b, d) && (this.rows.push(a), a = new Blockly.blockRendering.InputRow(this.constants_));
...
Blockly.blockRendering.RenderInfo.prototype.populateTopRow_ = function() {
	...
	var a = this.block_.hat ? "cap" === this.block_.hat : Blockly.BlockSvg.START_HAT,

		b = !!this.block_.previousConnection;
	...
	a ? (a = new Blockly.blockRendering.Hat(this.constants_), this.topRow.elements.push(a), this.topRow.capline = a.ascenderHeight) :

		b && (this.topRow.hasPreviousConnection = !0, this.topRow.connection = new Blockly.blockRendering.PreviousConnection(this.constants_, this.block_.previousConnection), this.topRow.elements.push(this.topRow.connection));

	this.block_.inputList.length && this.block_.inputList[0].type == Blockly.NEXT_STATEMENT && !this.block_.isCollapsed() ? this.topRow.minHeight = this.constants_.LARGE_PADDING : this.topRow.minHeight = this.constants_.MEDIUM_PADDING
...
Blockly.blockRendering.RenderInfo.prototype.populateBottomRow_ = function() {
	...
	this.bottomRow.minHeight = this.block_.inputList.length && this.block_.inputList[this.block_.inputList.length - 1].type == Blockly.NEXT_STATEMENT ? this.constants_.LARGE_PADDING : this.constants_.MEDIUM_PADDING - 1;
...
Blockly.blockRendering.RenderInfo.prototype.addInput_ = function(a, b) {
	...
	this.isInline && a.type == Blockly.INPUT_VALUE ? (b.elements.push(new Blockly.blockRendering.InlineInput(this.constants_, a)), b.hasInlineInput = !0) : a.type == Blockly.NEXT_STATEMENT ? (b.elements.push(new Blockly.blockRendering.StatementInput(this.constants_, a)), b.hasStatement = !0) : a.type == Blockly.INPUT_VALUE ? (b.elements.push(new Blockly.blockRendering.ExternalValueInput(this.constants_, a)), b.hasExternalInput = !0) : a.type == Blockly.DUMMY_INPUT && (b.hasDummyInput = !0)

};

Blockly.blockRendering.RenderInfo.prototype.shouldStartNewRow_ = function(a, b) {

	return b ? a.type == Blockly.NEXT_STATEMENT || b.type == Blockly.NEXT_STATEMENT ? !0 : a.type == Blockly.INPUT_VALUE || a.type == Blockly.DUMMY_INPUT ? !this.isInline : !1 : !1
...
Blockly.blockRendering.RenderInfo.prototype.addElemSpacing_ = function() {
	...
	for (var a = 0, b; b = this.rows[a]; a++) {
		...
		b.endsWithElemSpacer() && b.elements.push(new Blockly.blockRendering.InRowSpacer(this.constants_,
...
Blockly.blockRendering.RenderInfo.prototype.getInRowSpacing_ = function(a, b) {
	...
	return a && Blockly.blockRendering.Types.isLeftSquareCorner(a) && b && (Blockly.blockRendering.Types.isPreviousConnection(b) || Blockly.blockRendering.Types.isNextConnection(b)) ?
...
Blockly.blockRendering.RenderInfo.prototype.alignStatementRow_ = function(a) {
	...
	var b = a.getLastInput(),
		...
		d = this.statementEdge - this.startX;
...
Blockly.blockRendering.RenderInfo.prototype.getElemCenterline_ = function(a, b) {
	...
	if (Blockly.blockRendering.Types.isBottomRow(a)) {
		...
		return Blockly.blockRendering.Types.isNextConnection(b) ? c + b.height / 2 : c - b.height / 2

	}

	return Blockly.blockRendering.Types.isTopRow(a) ? Blockly.blockRendering.Types.isHat(b) ? a.capline - b.height / 2 : a.capline + b.height / 2 : a.yPos + a.height / 2

};

Blockly.blockRendering.RenderInfo.prototype.recordElemPositions_ = function(a) {

	for (var b = a.xPos, c = 0, d; d = a.elements[c]; c++) Blockly.blockRendering.Types.isSpacer(d) && (d.height = a.height), d.xPos = b, d.centerline = this.getElemCenterline_(a, d), b += d.width
...
Blockly.blockRendering.Drawer.prototype.drawTop_ = function() {
	...
	for (var c = 0, d; d = b[c]; c++) Blockly.blockRendering.Types.isLeftRoundedCorner(d) ? this.outlinePath_ += this.constants_.OUTSIDE_CORNERS.topLeft : Blockly.blockRendering.Types.isPreviousConnection(d) ? this.outlinePath_ += d.shape.pathLeft : Blockly.blockRendering.Types.isHat(d) ? this.outlinePath_ += this.constants_.START_HAT.path :

		Blockly.blockRendering.Types.isSpacer(d) && (this.outlinePath_ += Blockly.utils.svgPaths.lineOnAxis("h", d.width));
...
Blockly.blockRendering.Drawer.prototype.drawBottom_ = function() {
	...
	for (var c = b.length - 1, d; d = b[c]; c--) Blockly.blockRendering.Types.isNextConnection(d) ? this.outlinePath_ += d.shape.pathRight : Blockly.blockRendering.Types.isLeftSquareCorner(d) ? this.outlinePath_ += Blockly.utils.svgPaths.lineOnAxis("H", a.xPos) : Blockly.blockRendering.Types.isLeftRoundedCorner(d) ? this.outlinePath_ +=

		this.constants_.OUTSIDE_CORNERS.bottomLeft : Blockly.blockRendering.Types.isSpacer(d) && (this.outlinePath_ += Blockly.utils.svgPaths.lineOnAxis("h", -1 * d.width))
...
Blockly.blockRendering.Renderer.prototype.makeCursorDrawer = function(a, b) {

	return new Blockly.CursorSvg(a, b)

};

Blockly.blockRendering.Renderer.prototype.makePathObject = function(a) {

	return new Blockly.blockRendering.PathObject(a)
...
Blockly.geras.Highlighter.prototype.drawJaggedEdge_ = function(a) {
	...
	this.info_.RTL && (this.steps_ += Blockly.utils.svgPaths.lineOnAxis("H", a.width - this.highlightOffset_), this.steps_ += this.jaggedTeethPaths_.pathLeft, this.steps_ += Blockly.utils.svgPaths.lineOnAxis("v", a.height - this.jaggedTeethPaths_.height - this.highlightOffset_))
...
Blockly.geras.PathObject = function(a) {
...
Blockly.geras.PathObject.prototype.setPaths = function(a, b) {
...
Blockly.geras.RenderInfo.prototype.addInput_ = function(a, b) {
	...
	this.isInline && a.type == Blockly.INPUT_VALUE ? (b.elements.push(new Blockly.geras.InlineInput(this.constants_, a)), b.hasInlineInput = !0) : a.type == Blockly.NEXT_STATEMENT ? (b.elements.push(new Blockly.geras.StatementInput(this.constants_, a)), b.hasStatement = !0) : a.type == Blockly.INPUT_VALUE ? (b.elements.push(new Blockly.blockRendering.ExternalValueInput(this.constants_, a)), b.hasExternalInput = !0) : a.type == Blockly.DUMMY_INPUT && (b.hasDummyInput = !0)
...
Blockly.geras.RenderInfo.prototype.getInRowSpacing_ = function(a, b) {
	...
	if (Blockly.blockRendering.Types.isInlineInput(a) && b && !Blockly.blockRendering.Types.isInput(b)) return b.isEditable ? this.constants_.MEDIUM_PADDING : this.constants_.LARGE_PADDING;
	...
	if (Blockly.blockRendering.Types.isLeftRoundedCorner(a) && b) {
		...
		if (Blockly.blockRendering.Types.isNextConnection(b)) return c = (this.RTL ? 1 : -1) * this.constants_.DARK_PATH_OFFSET / 2, b.notchOffset -

			this.constants_.CORNER_RADIUS + c

	}

	return !Blockly.blockRendering.Types.isInput(a) && b && !Blockly.blockRendering.Types.isInput(b) && a.isEditable == b.isEditable || b && Blockly.blockRendering.Types.isJaggedEdge(b) ? this.constants_.LARGE_PADDING : this.constants_.MEDIUM_PADDING

};

Blockly.geras.RenderInfo.prototype.addAlignmentPadding_ = function(a, b) {
	...
	var e = a.getLastInput();

	e ? e.align == Blockly.ALIGN_LEFT ? d.width += b : e.align == Blockly.ALIGN_CENTRE ? (c.width += b / 2, d.width += b / 2) : e.align == Blockly.ALIGN_RIGHT && (c.width += b) : d.width += b;
...
Blockly.geras.Drawer.prototype.drawRightSideRow_ = function(a) {
	...
	Blockly.geras.Drawer.superClass_.drawRightSideRow_.call(this, a)
...
Blockly.geras.Renderer.prototype.makePathObject = function(a) {

	return new Blockly.geras.PathObject(a)
...
Blockly.Toolbox.prototype.init = function() {
	...
	Blockly.bindEventWithChecks_(this.HtmlDiv, "mousedown", this, function(a) {
		...
		Blockly.utils.isRightButton(a) ||
			...
			a.target == this.HtmlDiv ? Blockly.hideChaff(!1) : Blockly.hideChaff(!0);
...
Blockly.Toolbox.prototype.handleAfterTreeSelected_ = function(a, b) {
	...
	if (a != b && a != this) {
		...
		var c = new Blockly.Events.Ui(null, "category", a && a.getText(),
...
Blockly.Themes.Dark = new Blockly.Theme(Blockly.Themes.Dark.defaultBlockStyles, Blockly.Themes.Dark.categoryStyles);
...
Blockly.Themes.HighContrast = new Blockly.Theme(Blockly.Themes.HighContrast.defaultBlockStyles, Blockly.Themes.HighContrast.categoryStyles);
